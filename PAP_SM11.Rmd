...
Since the release of {admiral} 1.0, the pharmaverse philosophy of "as-standard-as-possible, as-custom-as-necessary" has left every sponsor company with the same question: how do we implement the last mile of standardisation — the sponsor-specific 10 to 20% of ADaM programming that {admiral} deliberately leaves open — without drowning our programming teams in bespoke, hard-to-maintain code? At Roche, the answer has evolved through three distinct stages, and this evolution is the spine of both the accompanying presentation and this paper.

We began, like most companies, with **code**: a closed-source extension package, {admiralroche}, providing static ADaM templates that new studies could copy and adapt. As the template library grew past twenty scripts, the maintenance burden of keeping duplicated logic in sync became untenable, so we moved to **code that writes code**: an internal package, {scriptr}, that lets developers define reusable, parameterised chunks of R code once and assemble them programmatically into templates, formatted and commented as if a human had written them by hand. Most recently we have taken a further step towards **code that writes code that writes code**: a companion package, {admiralrochetools}, that walks a programmer through a short interactive dialogue, reads the study's own Data Analysis Plan (DAP) machine-readable specification, and then uses {scriptr} to *generate a bespoke, ready-to-run ADaM template* tailored to that specific study's derivation choices.

This paper accompanies the talk of the same name and goes beyond what the slides could show. In particular, it gives a deeper account of how {scriptr} works internally — its chunk/call data model, its templating syntax, and its script-assembly engine — as a worked example for any organisation considering building a similar tool. It also explains, using version history and package metadata not shown in the talk, why the templating functionality was ultimately split out of {admiralroche} into the separate {admiralrochetools} package, and what that split means for package size, maintainability, and regulatory submission scope. We close with our reflections on the journey, recommendations for other companies starting down the same road, and pointers for further work — including where large language models might plausibly join this pipeline next.


# INTRODUCTION

Statistical programmers writing Analysis Data Model (ADaM) datasets for clinical trials increasingly do so using the {admiral} package and its wider "pharmaverse" ecosystem [pharmaverse.org]. {admiral} deliberately does not attempt to be a complete, drop-in solution: it exposes composable, reusable derivation functions and leaves the exact assembly of a study's ADaM datasets to the sponsor, since the fine details of how any given company or study implements a standard inevitably vary. This is by design, and it is a design that has been discussed at previous PHUSE conferences by our own colleagues. Ross Farrugia's 2024 paper on {admiral}'s open-source-to-company journey lays out the rationale for company-specific extension packages built on top of {admiral} [OS05], and Edoardo Mancini's 2024 paper on wearing "two hats" as both an {admiral} developer and an {admiral} user at a sponsor company describes the practical texture of building and maintaining such an extension day to day [OS07]. Rather than duplicate that material, this paper builds on it directly and, where useful, quotes it.

At Roche, the company extension package is {admiralroche}. This paper — and the talk it accompanies — tells the story of how the way we *use* {admiralroche} has changed over roughly two years, moving through three distinct stages that give the talk its title:
...
3. **Code that writes code that writes code.** A companion package, {admiralrochetools}, that combines {scriptr}'s assembly engine with a short interactive questionnaire and a study's own machine-readable Data Analysis Plan (DAP) specification, so that running a single function produces a template already tailored to that study's derivation choices — no manual search-and-replace required.

The presentation walks through this arc at the level a conference audience can absorb in twenty minutes: what problem each stage solves, and a short worked example of each. This paper has more room to breathe, and covers three things the talk could only gesture at:

- A closer look at **how {scriptr} is actually built** internally — its data model, its templating syntax, and its assembly engine — written so that a reader considering a similar tool at their own organisation has a genuine starting point (Section 4).
- The story of **why the dynamic template-generation logic now lives in a second package**, {admiralrochetools}, separate from {admiralroche} itself, including the concrete package-size and submission-scope reasoning behind that split (Section 5).
- A handful of **other details from the {admiralroche} and {admiralrochetools} documentation** that did not make the slides but which we think are of independent interest to the pharmaverse community (scattered throughout, and summarised in Section 7).
